...
function saveItems(quotationId: number, items: LineItemInput[], taxType: 'none' | 'cgst_sgst' | 'igst') {
  const totals = computeTotals(items, taxType);
  db.prepare('DELETE FROM quotation_items WHERE quotation_id = ?').run(quotationId);
  const ins = db.prepare(
    `INSERT INTO quotation_items (quotation_id, product_id, description, hsn_code, qty, unit, unit_price, tax_pct, amount, sort_order)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  totals.items.forEach((it, i) =>
    ins.run(quotationId, it.product_id ?? null, it.description, it.hsn_code ?? '', it.qty ?? null, it.unit ?? 'unit', it.unit_price, it.tax_pct ?? 0, it.amount, i)
  );
...
quotationsRouter.post('/', (req, res) => {
  ...
  const result = transaction(() => {
    ...
    const info = db.prepare(
      `INSERT INTO quotations (number, revision, date, customer_id, enquiry_id, currency, validity_date, payment_terms, delivery_terms, notes, tax_type, status)
       VALUES (?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')`
    ).run(
      number,
    ...
    saveItems(id, (body.items ?? []) as LineItemInput[], taxType);
    if (body.enquiry_id) {
...
quotationsRouter.put('/:id', (req, res) => {
  ...
  transaction(() => {
    db.prepare(
      `UPDATE quotations SET date = ?, customer_id = ?, enquiry_id = ?, currency = ?, validity_date = ?, payment_terms = ?, delivery_terms = ?, notes = ?, tax_type = ? WHERE id = ?`
    ).run(
    ...
    if (Array.isArray(body.items)) saveItems(id, body.items as LineItemInput[], taxType);
  });
  res.json(getFull(id));
});
...
quotationsRouter.post('/:id/revise', (req, res) => {
  ...
  const newId = transaction(() => {
    ...
    const info = db.prepare(
      `INSERT INTO quotations (number, revision, date, customer_id, enquiry_id, currency, validity_date, payment_terms, delivery_terms, notes, tax_type, status, subtotal, tax_total, grand_total)
       SELECT number, ?, ?, customer_id, enquiry_id, currency, validity_date, payment_terms, delivery_terms, notes, tax_type, 'negotiating', subtotal, tax_total, grand_total
       FROM quotations WHERE id = ?`
    ).run(maxRev.r + 1, new Date().toISOString().slice(0, 10), id);
    const newId = Number(info.lastInsertRowid);
    db.prepare(
      `INSERT INTO quotation_items (quotation_id, product_id, description, hsn_code, qty, unit, unit_price, tax_pct, amount, sort_order)
       SELECT ?, product_id, description, hsn_code, qty, unit, unit_price, tax_pct, amount, sort_order FROM quotation_items WHERE quotation_id = ?`
    ).run(newId, id);
