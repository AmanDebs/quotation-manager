...
export function buildPackingListPdf(id: number): TDocumentDefinitions {
  ...
  const totalsFor = (rows: Row[], title: string, fill: string) => headerCells.map((h: Cell) => {
    ...
    if (label === 'Description of Goods') return cell(title, 'left');
    if (label === 'Quantity') return cell(qtyTotal(rows));
    if (label === 'Boxes') return cell(packagesTotal(rows.map((it) => String(it.packages || ''))), 'center');
    // Only the lines that actually show a thousand-pieces figure are in it.
    if (label === "Qty in Thousand Pcs") {
      const t = rows.reduce((sum, it) => sum + (thousands(it) ?? 0), 0);
      return cell(t ? fmtNum(t, 2) : '');
    }
    if (label === 'Net Wt (kg)') return cell(fmtNum(round2(rows.reduce((sum, it) => sum + (it.net_weight || 0), 0))));
