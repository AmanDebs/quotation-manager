...
- **The dashboard is ordered by urgency**: a "Needs attention" strip (overdue follow-ups/orders/invoices, lapsed quotation validity, pending approvals), then money tiles, then charts and tables. The attention strip and the receivables ageing **deliberately ignore the date-range filter** — an overdue item from months ago is the most urgent kind, not the least. The currency selector in the page header drives every money figure on the page. Ageing buckets (0-30/31-60/61-90/90+ by invoice age) must always sum to the receivables outstanding total; that cross-check is the quickest way to catch a regression.
